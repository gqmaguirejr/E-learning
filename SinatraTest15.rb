...
#$base_url = "https://#{host}/api/v1/users/self/custom_data/program_of_study"
base_url = "http://#{host}/api/v1/users/self/custom_data/program_of_study"
base_url_prefix = "http://#{host}/api/v1/users/sis_user_id:"
base_url_postfix = "/custom_data/program_of_study"
user_base_url = "http://#{host}/api/v1/users/self/profile"
user_base_url_prefix = "http://#{host}/api/v1/users/sis_user_id:"
user_base_url_postfix = "/profile"
...
get '/getProgramData' do
        ...
        @getResponse = HTTParty.get(user_base_url, :headers => $header )
        puts "user associated with the TOKEN: getResponse.body user_base_url is #{@getResponse}"

        @url_to_use=user_base_url_prefix+session['lis_person_sourcedid']+user_base_url_postfix
	...
	@payload={"ns" => "se.kth.canvas-app.program_of_study"}
        ...
        @url_to_use=base_url_prefix+session['lis_person_sourcedid']+base_url_postfix
        puts "url_to_use is #{@url_to_use}"
...
get '/getGeneralData' do
        ...
        planned_start_max=planned_start_today + (356*24*60*60)
...
post '/Outcome' do
  ...
  if params.has_key?('selected_examiner') 
    ...
    puts "selected_examiner is #{@selected_examiner}"
  else
  ...
  <html > 
	...
	<body > 
        <p><span lang="en">Thank you for selecting course code #{@selected_course} and examiner #{@selected_examiner}.</span> | <span lang="sv">Tack för att du valt kurskod #{@selected_course} och examinator #{@selected_examiner}.</span<p>
        <p><span lang="en">You have finished the replacement for the paper form. Best of success in your degree project.</span> | <span lang="sv">Du har slutfört ersättningen för pappersblanket. Bäst av framgång i ditt examensarbete.</span></p> 
	</body > 
   </html > 
   HTML
