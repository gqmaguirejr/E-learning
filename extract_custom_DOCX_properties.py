...
#
# Purpose: The program extract the list of custom docproperties and their values from a DOCX file
#
# Output: outputs the properties in a JSON file: <input_filename>-extracted.json
#
# The custom DOCPROPETIES are in a file with the name docProps/custom.xml
#
# Example:
...
#
# Notes:
#    Only one test file has been run.
#
# 2021-12-12 G. Q. Maguire Jr.
# Base on earlier JSON_to_DOCX_cover.py
#
import re
import sys
import subprocess
...
def extract_properties_from_file(xml_content):
    global Verbose_Flag
    custom_properties=dict()    # the extracted properties and their values
    ...
    while offset >= 0:
        pattern2='pid="'
        pattern2_end='"'
        pid_offset=xml_content.find(pattern2, offset)
        pid_end=xml_content.find(pattern2_end, pid_offset+len(pattern2))
        pid=xml_content[pid_offset+len(pattern2):pid_end]
        #
        pattern3='name="'
        pattern3_end='"'
        name_offset=xml_content.find(pattern3, pid_end+len(pattern2_end))
        name_end=xml_content.find(pattern3_end, name_offset+len(pattern3))
        name=xml_content[name_offset+len(pattern3):name_end]
        #
        pattern4='<vt:lpwstr>'
        pattern4_end='</vt:lpwstr>'
        value_offset=xml_content.find(pattern4, name_end+len(pattern3_end))
        value_end=xml_content.find(pattern4_end, value_offset+len(pattern4))
        value=xml_content[value_offset+len(pattern4):value_end]
        if Verbose_Flag:
            print("pid={0}, name={1}, value={2}".format(pid, name, value))
        entry=dict()
        entry[name]=value
        custom_properties[pid]=entry
        offset=xml_content.find(pattern1, value_end+len(pattern4_end))
...
def main(argv):
    ...
    argp = argparse.ArgumentParser(description="extract_custom_DOCX_properties.py: to extract custom DOCPROPERTIES from a DOCX file")
    ...
    xml_content = document.read(word_document_file_name).decode('utf-8')
    document.close()
    custom_properties = extract_properties_from_file(xml_content)

    output_filename="{}-extracted.json".format(input_filename[:-5])
    print("outputting extracted data to {}".format(output_filename))

    with open(output_filename, 'w', encoding='utf-8') as output_FH:
        j_as_string = json.dumps(custom_properties, ensure_ascii=False)
